...
describe("Matrix3x2", () => {
    describe("constructor", () => {
        ...
        it("initializes matrix with given elements", () => {
            const m = new Matrix3x2(1, 2, 3, 4, 5, 6);
            expect(m.toJSON()).toEqual([
                 1, 2,
                 3, 4,
                 5, 6
        ...
        it("initializes matrix from a buffer without offset", () => {
            ...
            const m = new Matrix3x2(array.buffer);
        ...
        it("initializes matrix from a buffer with offset", () => {
            ...
            const m = new Matrix3x2(array.buffer, 4);
            m.setComponents(1, 2, 3, 4, 5, 6);
    ...
    describe("fromMatrix", () => {
        it("initializes matrix from other 2x2 matrix", () => {
            ...
            const b = Matrix3x2.fromMatrix(a);
    ...
    describe("m* getters", () => {
        it("reads the matrix components", () => {
            const m = new Matrix3x2(2, 3, 4, 5, 6, 7);
            expect(m.m11).toBe(2);
            expect(m.m12).toBe(3);
            expect(m.m21).toBe(4);
            expect(m.m22).toBe(5);
    ...
    describe("clone", () => {
        it("returns new matrix", () => {
            const matrix = new Matrix3x2(6, 7, 1, 2, 3, 4);
            const clone = matrix.clone();
            expect(clone).toBeInstanceOf(Matrix3x2);
    ...
    describe("fromJSON", () => {
        it("constructs matrix from component array", () => {
            const m = Matrix3x2.fromJSON([ 2, 3, 4, 5, 6, 7 ]);
            expect(m).toBeInstanceOf(Matrix3x2);
    ...
    describe("equals", () => {
        it("correctly implements the equality contract", () => {
            expect(new Matrix3x2(1, 2, 3, 4, 5, 6,)).toBeEquatable([
                ...
                new Matrix3x2(1, 0, 3, 4, 5, 6),
                new Matrix3x2(1, 2, 0, 4, 5, 6),
                new Matrix3x2(1, 2, 3, 0, 5, 6),
                new Matrix3x2(1, 2, 3, 4, 0, 6),
                new Matrix3x2(1, 2, 3, 4, 5, 0)
            ]);
        });
    });
    ...
    describe("isIdentity", () => {
        ...
        it("returns false if matrix is not an identity matrix", () => {
            expect(new Matrix3x2(2, 0, 0, 1, 0, 0).isIdentity()).toBe(false);
            expect(new Matrix3x2(1, 2, 0, 1, 0, 0).isIdentity()).toBe(false);
            expect(new Matrix3x2(1, 0, 2, 1, 0, 0).isIdentity()).toBe(false);
            expect(new Matrix3x2(1, 0, 0, 2, 0, 0).isIdentity()).toBe(false);
            expect(new Matrix3x2(1, 0, 0, 1, 2, 0).isIdentity()).toBe(false);
            expect(new Matrix3x2(1, 0, 0, 1, 0, 2).isIdentity()).toBe(false);
        });
    });
    ...
    describe("add", () => {
        ...
        it("adds given matrix", () => {
            const a = new Matrix3x2(20, 3, 40, 5, 60, 7);
            const b = new Matrix3x2(3, 40, 5, 60, 7, 80);
    ...
    describe("sub", () => {
     ...
     describe("compMul", () => {
        it("multiplies matrix with a factor", () => {
            const m = new Matrix3x2(20, -3, 40, -5, 60, -7);
